    ...
    async def __call__(self, request: Request) -> Optional[str]:
        ...
        if not authorization or scheme.lower() != "basic":
            if self.auto_error:
                ...
                # return response
                raise HTTPException(headers={"WWW-Authenticate": "Basic"}, status_code=401, detail="")
            else:
                return None
        return param
...
async def get_current_active_user(current_user: User = Depends(get_current_user)):
    if current_user.disabled:
        raise HTTPException(status_code=400, detail="Inactive user login")
    return current_user


@app.get("/")
async def homepage():
    return "Login admin for successfull access!"
